...
const USAGE_QUOTA: { label: string; used: number; quota: number; unit: string; icon: UsageIconKey }[] = [
...
const TRACKING_ITEMS: TrackingItem[] = [
  {
    id: 1, creator: "@skincare_sam", avatar: "S", seed: "skincare_sam",
    platform: "TikTok",
    title: "XX 品牌深度测评 · 晚间护肤 routine",
    publishedAt: "04-20", cost: "$2,800",
    views: "180K", viewsRaw: 180000, dailyDelta: "+8K/日",
    likes: "12K", likesRaw: 12000,
    comments: "890", commentsRaw: 890,
    shares: "2.3K", sharesRaw: 2300,
    cpm: "$15.6", cpe: "$0.13",
    avgCPM: "$19.6", cpmDelta: "optimal",
    trend: "growing", trendLabel: "增长中 · 日均 +8K 播放",
    chartData: [20, 45, 80, 120, 150, 165, 180],
    stoppedAt: null,
  },
  {
    id: 2, creator: "@fit_jenny", avatar: "F", seed: "fit_jenny",
    platform: "TikTok",
    title: "健身后必备好物分享",
    publishedAt: "04-18", cost: "$720",
    views: "95K", viewsRaw: 95000, dailyDelta: "+0.5K/日",
    likes: "7.8K", likesRaw: 7800,
    comments: "562", commentsRaw: 562,
    shares: "1.8K", sharesRaw: 1800,
    cpm: "$7.6", cpe: "$0.07",
    avgCPM: "$19.6", cpmDelta: "optimal",
  ...
  {
    id: 3, creator: "@glow_girl", avatar: "G", seed: "glow_girl",
    platform: "Instagram",
    title: "我的晚间护肤 routine",
    publishedAt: "04-22", cost: "$1,500",
    views: "42K", viewsRaw: 42000, dailyDelta: "增速骤停",
    likes: "2.1K", likesRaw: 2100,
    comments: "98", commentsRaw: 98,
    shares: "320", sharesRaw: 320,
    cpm: "$35.7", cpe: "$0.59",
    avgCPM: "$19.6", cpmDelta: "worse",
    trend: "anomaly", trendLabel: "异常 · 48 小时播放增速骤停",
    chartData: [10, 28, 38, 40, 41, 41, 42],
    stoppedAt: null,
  },
  {
    id: 4, creator: "@style_nina", avatar: "N", seed: "style_nina",
    platform: "Instagram",
    title: "春夏穿搭 · 通勤 lookbook",
    publishedAt: "04-14", cost: "$900",
    views: "31K", viewsRaw: 31000, dailyDelta: "—",
    likes: "1.5K", likesRaw: 1500,
    comments: "210", commentsRaw: 210,
    shares: "450", sharesRaw: 450,
    cpm: "$29.0", cpe: "$0.44",
    avgCPM: "$19.6", cpmDelta: "worse",
    trend: "stopped", trendLabel: "已停止 · 停止于 04-22",
...
function TrendLineChart({ data, color }: { data: number[]; color: string }) {
  ...
  const areaPath = `M ${pts[0]} ` + pts.slice(1).map(p => `L ${p}`).join(" ") + ` L ${(data.length - 1) * step},${h} L 0,${h} Z`;
  return (
    <svg width="100%" viewBox={`0 0 ${w} ${h}`} className="overflow-visible">
      ...
      {data.map((v, i) => (
        <circle key={i} cx={i * step} cy={h - ((v - min) / range) * (h - 8) - 4}
...
function BentoAreaChart({ data, color }: { data: number[]; color: string }) {
  ...
  return (
    <svg width="100%" viewBox={`0 0 ${w} ${h}`} preserveAspectRatio="none" className="h-full w-full">
      <defs>
...
function InteractionDonut({ likes, comments, shares }: { likes: number; comments: number; shares: number }) {
  ...
  return (
    <svg width="72" height="72" viewBox="0 0 72 72" className="shrink-0">
      ...
      <circle cx="36" cy="36" r={r} fill="none" stroke="#c96442" strokeWidth="8"
        strokeDasharray={`${likeLen} ${c}`} strokeLinecap="butt"
        transform="rotate(-90 36 36)" />
      <circle cx="36" cy="36" r={r} fill="none" stroke="#8a9a7a" strokeWidth="8"
        strokeDasharray={`${commentLen} ${c}`}
        strokeDashoffset={-likeLen}
        strokeLinecap="butt"
        transform="rotate(-90 36 36)" />
      <circle cx="36" cy="36" r={r} fill="none" stroke="#b8a88a" strokeWidth="8"
...
function YouTubeGlyph() {
  return (
    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" aria-hidden="true">
      <path d="M21.3 7.2a2.8 2.8 0 0 0-2-2C17.6 4.7 12 4.7 12 4.7s-5.6 0-7.3.5a2.8 2.8 0 0 0-2 2A29.5 29.5 0 0 0 2.2 12c0 1.6.2 3.2.5 4.8a2.8 2.8 0 0 0 2 2c1.7.5 7.3.5 7.3.5s5.6 0 7.3-.5a2.8 2.8 0 0 0 2-2c.3-1.6.5-3.2.5-4.8s-.2-3.2-.5-4.8Z" fill="currentColor" />
...
// ── Project data ───────────────────────────────────────────────────────────────
interface Project {
  name: string; creatorCount: number; dateRange: string;
...
const INIT_PROJECTS: Project[] = [
  {
    name: "Q2夏季 Campaign", creatorCount: 96, dateRange: "04-01/06-30",
    kpi: [
    ...
    product: { name: "防蓝光护眼面霜", category: "美妆护肤", brand: "MyBrand", link: "https://www.mybrand.com/product" },
    platforms: ["TikTok", "Instagram"],
  },
  {
    name: "美妆博主池", creatorCount: 5, dateRange: "",
    kpi: [
    ...
    product: { name: "持久遮瑕粉底液", category: "彩妆", brand: "GlowLab", link: "https://www.glowlab.com/product" },
    platforms: ["TikTok", "Instagram", "YouTube"],
  },
];

// ── Detail Drawer ──────────────────────────────────────────────────────────────
function DetailDrawer({ item, onClose }: { item: typeof TRACKING_ITEMS[0]; onClose: () => void }) {
  const { openCreatorProfile } = useCreatorProfile();
  const [range, setRange] = useState<"7天" | "30天">("7天");
  const trendColor = item.trend === "growing" ? "#10b981" : item.trend === "anomaly" ? "#f59e0b" : "#87867f";

  const data30 = [item.chartData[0], ...item.chartData,
    item.chartData[6] + 5, item.chartData[6] + 12,
    item.chartData[6] + 8, item.chartData[6] + 18,
    item.chartData[6] + 15, item.chartData[6] + 22,
    item.chartData[6] + 19, item.chartData[6] + 28,
    item.chartData[6] + 25, item.chartData[6] + 24,
    item.chartData[6] + 30, item.chartData[6] + 32,
    item.chartData[6] + 29, item.chartData[6] + 35,
    item.chartData[6] + 33, item.chartData[6] + 38,
    item.chartData[6] + 36, item.chartData[6] + 42,
    item.chartData[6] + 38, item.chartData[6] + 44,
    item.chartData[6] + 40,
  ];
      ...
      <div className="fixed right-0 top-0 z-50 flex h-full w-[480px] flex-col bg-white shadow-2xl">
        ...
        <div className="flex items-start justify-between border-b border-[#e8e6dc] px-6 py-5">
          <div className="flex items-center gap-4">
            <button type="button" onClick={() => openCreatorProfile(trackingItemToProfile(item))} className="h-12 w-12 shrink-0 overflow-hidden rounded-full ring-1 ring-[#e8e6dc] transition-transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-[#c96442] focus:ring-offset-2">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={avatarUrl(item.seed)} alt={item.creator}
                className="h-full w-full object-cover" loading="lazy" />
            </button>
            <div>
              <div className="flex items-center gap-2">
                <span className="font-semibold text-[#141413]">{item.creator}</span>
                <span className="rounded-full bg-[#f5f4ed] px-2 py-0.5 text-[10px] text-[#87867f]">{item.platform}</span>
              </div>
              <p className="mt-0.5 text-xs text-[#87867f]">发布 {item.publishedAt} · 费用 {item.cost}</p>
            </div>
          </div>
        ...
        <div className="flex-1 space-y-6 overflow-y-auto px-6 py-5">
          ...
          <div>
            ...
            <div className="overflow-hidden rounded-2xl border border-[#e8e6dc] bg-[#faf9f5]">
              ...
              <div className="relative h-48 w-full overflow-hidden bg-[#f5f4ed]">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={thumbnailUrl(item.seed)} alt={item.title}
                  className="h-full w-full object-cover" loading="lazy" />
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="flex h-10 w-10 items-center justify-center rounded-full bg-white/90 backdrop-blur-sm ring-1 ring-white/40">
              ...
              <div className="flex items-start justify-between gap-3 px-4 py-3">
                ...
                <a href="#" className="flex shrink-0 items-center gap-1 text-xs text-[#c96442] hover:underline">
                  查看原帖<ExternalLink className="h-3 w-3" />
                </a>
              </div>
              <p className="px-4 pb-3 text-[10px] text-[#87867f]">发布时间：{item.publishedAt} · 发布平台：{item.platform}</p>
            </div>
          </div>
          ...
          <div>
                ...
                {(["7天", "30天"] as const).map(r => (
                  <button key={r} type="button" onClick={() => setRange(r)}
                    className={cn(
                      ...
                      range === r ? "bg-[#c96442] text-white" : "bg-[#f5f4ed] text-[#87867f] hover:bg-[#e8e6dc]"
                    )}>
                    {r}
                  </button>
                ))}
            ...
            <div className="rounded-2xl border border-[#e8e6dc] bg-white px-4 pb-4 pt-4">
...
function ConfigDrawer({
  ...
  return (
    <>
      <div className="fixed inset-0 z-40 bg-black/20" onClick={onClose} />
      <div className="fixed right-0 top-0 z-50 flex h-full w-[540px] flex-col bg-white shadow-2xl">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-6">
          ...
          <div>
            ...
            <div>
              ...
              <input value={projectName} onChange={e => { setProjectName(e.target.value); if (error) setError(null); }}
          ...
          <div>
            ...
            <div className="space-y-3">
              {[
                ...
                <div key={f.label}>
                  <label className="mb-1.5 block text-xs font-medium text-[#4d4c48]">{f.label}</label>
          ...
          <div>
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="min-w-0 flex-1">
                <p className="text-xs text-[#87867f]">建议尺寸 400×400px，支持 PNG、JPG、WebP 等图片格式</p>
                <div className="mt-3 flex flex-wrap items-center gap-2">
                  <button type="button" onClick={() => imageInputRef.current?.click()}
                    ...
                    <FileImage className="h-3.5 w-3.5" />上传图片
                  </button>
                  <button type="button" onClick={() => infoInputRef.current?.click()}
                    className="flex items-center gap-1.5 rounded-xl border border-[#e8e6dc] bg-white px-3 py-2 text-xs font-medium text-[#4d4c48] transition-colors hover:bg-[#f5f4ed] active:scale-[0.98]">
                    <FileUp className="h-3.5 w-3.5" />上传产品信息
                  </button>
                </div>
                <div className="mt-3 space-y-1.5 text-[11px] text-[#87867f]">
                  <p className="truncate">产品图片：{productImageName ?? "未上传"}</p>
                  <p className="truncate">产品信息：{productInfoName ?? "支持 PDF、PNG、JPG、截图等资料"}</p>
                </div>
                <input
                  ref={imageInputRef}
                  type="file"
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-3">
              {[
                ...
                return (
                  <button key={p.id} type="button" onClick={() => togglePlat(p.id)} title={p.label} aria-label={p.label}
                    className={cn(
                    ...
                    <span className="relative">
                      {p.icon}
                      {active && <span className="absolute -right-1.5 -top-1.5 h-2.5 w-2.5 rounded-full border border-white bg-[#c96442]" />}
                    </span>
                  </button>
                );
              })}
            </div>
            <p className="mt-2 text-[11px] text-[#87867f]">点击平台 Logo 可切换目标平台，当前支持 TikTok、Instagram、YouTube。</p>
          </div>
...
function BentoTrackingCard({
  item, onDetail, onStopRequest, onRestore,
}: {
  item: TrackingItem;
  onDetail: () => void;
  ...
  const PlatformGlyph = item.platform === "TikTok" ? TikTokGlyph
                      : item.platform === "Instagram" ? InstagramGlyph
                      : YouTubeGlyph;
    ...
    <div className={cn(
      ...
      <div className="flex items-center justify-between px-5 pt-5">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <button type="button" onClick={(e) => { e.stopPropagation(); openCreatorProfile(trackingItemToProfile(item)); }} className="relative h-11 w-11 shrink-0 overflow-hidden rounded-full ring-1 ring-[#e8e6dc] transition-transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-[#c96442] focus:ring-offset-2">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src={avatarUrl(item.seed)} alt={item.creator}
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              ...
              <span className="flex items-center gap-1 rounded-full bg-white/60 px-2 py-0.5 text-[10px] font-medium text-[#4d4c48] ring-1 ring-[#e8e6dc]">
                <span className="[&>svg]:h-3 [&>svg]:w-3"><PlatformGlyph /></span>
                {item.platform}
              </span>
              <span className={cn("flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-medium", style.tagBg)}>
            ...
            <p className="mt-0.5 text-[11px] text-[#87867f]">
              发布 {item.publishedAt} · 费用 <span className="font-medium text-[#4d4c48]">{item.cost}</span>
            </p>
          </div>
        </div>
        ...
        <div className="relative">
          ...
          {menuOpen && (
            <>
              ...
              <div className="absolute right-0 top-9 z-20 w-36 overflow-hidden rounded-xl border border-[#e8e6dc] bg-white shadow-lg">
                <button type="button" onClick={() => { setMenuOpen(false); onDetail(); }}
                  className="flex w-full items-center gap-2 px-3 py-2 text-xs text-[#4d4c48] hover:bg-[#faf9f5]">
                  <Eye className="h-3.5 w-3.5" />查看详情
                </button>
                {isStopped ? (
                  <button type="button" onClick={() => { setMenuOpen(false); onRestore(item); }}
                    className="flex w-full items-center gap-2 border-t border-[#e8e6dc] px-3 py-2 text-xs text-[#c96442] hover:bg-[#fbf5ed]">
                    <Play className="h-3.5 w-3.5" />重新追踪
                  </button>
                ) : (
                  <>
                    <button type="button"
                      className="flex w-full items-center gap-2 px-3 py-2 text-xs text-[#4d4c48] hover:bg-[#faf9f5]">
                      <ExternalLink className="h-3.5 w-3.5" />查看原帖
                    </button>
                    <button type="button" onClick={() => { setMenuOpen(false); onStopRequest(item); }}
                      className="flex w-full items-center gap-2 border-t border-[#e8e6dc] px-3 py-2 text-xs text-[#c96442] hover:bg-[#fbf5ed]">
                      <X className="h-3.5 w-3.5" />停止追踪
                    </button>
                  </>
                )}
              </div>
        ...
        <div className="flex items-start justify-between gap-3">
          <div>
            <div className="flex items-baseline gap-2">
              ...
              <span className={cn("text-[11px] font-medium", item.trend === "anomaly" ? "text-[#c96442]" : "text-[#5a6a4a]")}>
      ...
      <div className="grid grid-cols-2 gap-3 p-5 pt-3">
        ...
        <div className="rounded-2xl border border-[#e8e6dc] bg-white px-4 py-3.5">
          <p className="text-[10px] font-medium uppercase tracking-wide text-[#87867f]">互动</p>
          <div className="mt-2 flex items-center gap-3">
            <div className="relative">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <span className="text-[11px] font-semibold leading-none text-[#141413]">
            ...
            <div className="flex-1 space-y-1">
              <div className="flex items-center justify-between text-[11px]">
                <span className="flex items-center gap-1.5 text-[#87867f]">
                  <span className="inline-block h-1.5 w-1.5 rounded-full bg-[#c96442]" />点赞
                </span>
                <span className="font-medium text-[#4d4c48]">{item.likes}</span>
              </div>
              <div className="flex items-center justify-between text-[11px]">
                <span className="flex items-center gap-1.5 text-[#87867f]">
                  <span className="inline-block h-1.5 w-1.5 rounded-full bg-[#8a9a7a]" />评论
                </span>
                <span className="font-medium text-[#4d4c48]">{item.comments}</span>
              </div>
              <div className="flex items-center justify-between text-[11px]">
                <span className="flex items-center gap-1.5 text-[#87867f]">
                  <span className="inline-block h-1.5 w-1.5 rounded-full bg-[#b8a88a]" />分享
                </span>
        ...
        {/* 费用效率 Widget */}
        <div className="rounded-2xl border border-[#e8e6dc] bg-white px-4 py-3.5">
          <p className="text-[10px] font-medium uppercase tracking-wide text-[#87867f]">费用效率</p>
      ...
      <button type="button" onClick={onDetail}
        className="absolute bottom-5 right-5 rounded-xl border border-[#e8e6dc] bg-white/80 px-3 py-1.5 text-[11px] font-medium text-[#4d4c48] opacity-0 shadow-sm transition-opacity hover:bg-white hover:text-[#c96442] group-hover:opacity-100">
        查看详情 →
      </button>
    </div>
  );
...
function StopTrackingModal({ item, onCancel, onConfirm }: {
  item: TrackingItem;
  onCancel: () => void;
  onConfirm: () => void;
}) {
        ...
        <div className="px-6 pt-6 pb-4">
          ...
          <div className="mt-4 rounded-2xl border border-[#e8e6dc] bg-white/80 px-4 py-3">
            <div className="flex items-center gap-3">
              <button type="button" onClick={() => openCreatorProfile(trackingItemToProfile(item))} className="shrink-0 rounded-full transition-transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-[#c96442] focus:ring-offset-1">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={avatarUrl(item.seed)} alt={item.creator}
...
const MOCK_LIBRARY_CREATORS = [
  { handle: "@beauty_lily",   seed: "beauty_lily",   followers: "220K", status: "已合作",  complete: true,  category: "美妆护肤" },
  { handle: "@wellness_aria", seed: "wellness_aria", followers: "185K", status: "合作中",  complete: true,  category: "健康生活" },
  { handle: "@home_oliver",   seed: "home_oliver",   followers: "94K",  status: "已归档",  complete: false, category: "家居生活" },
  { handle: "@minimal_ruby",  seed: "minimal_ruby",  followers: "156K", status: "合作中",  complete: true,  category: "美妆护肤" },
  { handle: "@foodie_ken",    seed: "foodie_ken",    followers: "71K",  status: "已归档",  complete: false, category: "美食" },
] as const;
...
function AddTrackingModal({ onClose, onAdd }: {
  ...
  const handleLinkChange = (v: string) => {
    setLink(v);
    if (!v.trim()) { setPreview(null); setLoading(false); return; }
    setLoading(true);
    setPreview(null);
    setTimeout(() => {
      setLoading(false);
          ...
          {(["link", "library"] as const).map(t => (
            <button key={t} type="button" onClick={() => setTab(t)}
              className={cn(
                ...
                tab === t
                  ? "font-medium text-[#c96442]"
                  : "text-[#87867f] hover:text-[#4d4c48]"
              )}>
              {t === "link" ? "粘贴内容链接" : "从博主库选择"}
              {tab === t && <span className="absolute inset-x-3 bottom-0 h-0.5 rounded-full bg-[#c96442]" />}
            </button>
          ))}
        </div>
          ...
          {tab === "link" ? (
            <div className="space-y-4">
              <div>
                ...
                <input value={link} onChange={e => handleLinkChange(e.target.value)}
                  placeholder="粘贴 TikTok / Instagram / YouTube 链接..."
                  className="mt-1.5 w-full rounded-xl border border-[#e8e6dc] bg-white px-3.5 py-2.5 text-sm text-[#141413] placeholder:text-[#c8c7c3] focus:border-[#c96442]/40 focus:outline-none focus:ring-2 focus:ring-[#c96442]/15" />
                ...
                <div className="space-y-3">
                  <div className="rounded-2xl border border-[#e8e6dc] bg-white px-4 py-3.5">
                    <div className="flex gap-3">
                      {/* eslint-disable-next-line @next/next/no-img-element */}
                      <img src={thumbnailUrl(preview.seed)} alt=""
                        className="h-20 w-20 shrink-0 rounded-xl object-cover" />
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2">
                          <button type="button" onClick={() => openCreatorProfile({ name: preview.handle, handle: preview.handle, avatarUrl: avatarUrl(preview.handle.replace("@","")), region: "--", followers: "--", er: "--", platform: (preview.platform.toLowerCase() as "tiktok" | "instagram" | "youtube") })} className="shrink-0 rounded-full transition-transform hover:scale-110 focus:outline-none focus:ring-2 focus:ring-[#c96442] focus:ring-offset-1">
                            {/* eslint-disable-next-line @next/next/no-img-element */}
                            <img src={avatarUrl(preview.handle.replace("@",""))} alt=""
                              className="h-5 w-5 rounded-full ring-1 ring-[#e8e6dc]" />
                          </button>
                          <span className="text-xs font-medium text-[#4d4c48]">{preview.handle}</span>
                          <span className="rounded-full bg-[#faf9f5] px-2 py-0.5 text-[10px] text-[#87867f] ring-1 ring-[#e8e6dc]">{preview.platform}</span>
                        </div>
                        <p className="mt-1.5 line-clamp-2 text-sm font-medium text-[#141413]">{preview.title}</p>
                        <p className="mt-1 text-[11px] text-[#87867f]">发布 {preview.publishedAt}</p>
                      </div>
                    </div>
                  </div>
                  <div>
                    <label className="text-xs font-medium text-[#4d4c48]">合作费用</label>
                    <input defaultValue={preview.cost}
            ...
            <div className="space-y-2">
              <p className="mb-2 text-[11px] text-[#87867f]">仅展示「已归档 / 合作中」的博主。信息不全的博主无法追踪。</p>
              {MOCK_LIBRARY_CREATORS.map(c => (
                <div key={c.handle} className={cn(
                  "flex items-center gap-3 rounded-2xl border px-4 py-3 transition-colors",
                  c.complete
                    ? "cursor-pointer border-[#e8e6dc] bg-white hover:bg-[#faf9f5]"
                    : "border-dashed border-[#e8e6dc] bg-[#faf9f5]/50 opacity-60"
                )}>
                  <button type="button" onClick={() => openCreatorProfile({ name: c.handle, handle: c.handle, avatarUrl: avatarUrl(c.seed), region: "--", followers: c.followers, er: "--" })} className="shrink-0 rounded-full transition-transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-[#c96442] focus:ring-offset-1">
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img src={avatarUrl(c.seed)} alt={c.handle}
                      className="h-10 w-10 rounded-full ring-1 ring-[#e8e6dc]" />
                  </button>
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      <span className="truncate text-sm font-medium text-[#141413]">{c.handle}</span>
                      <span className="rounded-full bg-[#faf9f5] px-2 py-0.5 text-[10px] text-[#87867f] ring-1 ring-[#e8e6dc]">{c.status}</span>
                    </div>
                    <p className="mt-0.5 text-[11px] text-[#87867f]">{c.followers} 粉丝 · {c.category}</p>
                  </div>
                  {c.complete ? (
                    <button type="button" onClick={() => onAdd("library")}
...
function ProjectTab() {
  ...
  const filteredTracking = statusFilter === "all"
    ? tracking
    : tracking.filter(t => t.trend === statusFilter);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-2">
        {projects.map((project) => (
          <button key={project.id} type="button" onClick={() => selectProject(project.id)}
            ...
            <span className={cn("text-[11px]", currentProjectId === project.id ? "text-[#c96442]/70" : "text-[#87867f]")}>
              {(DEFAULT_PROJECT_CREATOR_COUNT_BY_ID[project.id] ?? 0)}位博主
              {formatProjectTimeline(project) !== "未设置时间范围" ? ` · ${formatProjectTimeline(project)}` : ""}
            </span>
          </button>
        ))}
        ...
        {/* 编辑项目配置 — 右对齐 */}
        <button type="button" onClick={() => openEditProject(currentProject.id)}
          className="ml-auto flex items-center gap-1.5 text-sm text-[#87867f] hover:text-[#c96442]">
          <Pencil className="h-3.5 w-3.5" />编辑项目配置
        </button>
      </div>
      ...
      <div className="rounded-3xl border border-[#e8e6dc] bg-white p-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-xs leading-5 text-[#87867f]">这些信息会同步给博主发现、博主库和建联中心，帮助团队围绕同一个项目执行。</p>
          </div>
          <button
            type="button"
        ...
        <div className="mt-3 rounded-2xl bg-[#faf9f5] px-4 py-3">
          ...
          <div className="mt-1 text-sm text-[#141413]">
            {currentProject.productLink ? (
              <a href={currentProject.productLink} target="_blank" rel="noreferrer" className="text-[#c96442] hover:underline">
                {currentProject.productLink}
              </a>
            ) : (
              "未填写"
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            {counts.anomaly > 0 && (
              <button type="button" onClick={() => setStatusFilter("anomaly")}
        ...
        {tracking.length === 0 ? (
          <div className="rounded-3xl border-2 border-dashed border-[#e8e6dc] bg-[#faf9f5]/60 py-20 text-center">
            ...
            <button type="button" onClick={() => setAddOpen(true)}
              ...
              <Plus className="h-3.5 w-3.5" />添加第一个
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            {filteredTracking.map(item => (
              <BentoTrackingCard key={item.id} item={item}
      ...
      {stopTarget && <StopTrackingModal item={stopTarget}
        onCancel={() => setStopTarget(null)} onConfirm={handleStopConfirm} />}
      {addOpen && <AddTrackingModal onClose={() => setAddOpen(false)}
...
function BillingTab() {
  return (
    <div className="w-full space-y-4">
      ...
      <div
        className="relative overflow-hidden rounded-2xl p-6 shadow-[0_8px_28px_-16px_rgba(20,20,19,0.12)] ring-1 ring-inset ring-white/70"
        style={{
          backgroundImage:
            "linear-gradient(135deg, #fbfaf6 0%, #f5f0e6 55%, #f0e6d8 100%)",
        }}
      >
        {/* Soft terracotta glow — top-right */}
        <div className="pointer-events-none absolute -right-20 -top-24 h-56 w-56 rounded-full bg-[#c96442]/12 blur-3xl" />
        ...
        <div className="relative flex items-start justify-between gap-6">
          <div className="min-w-0">
            {/* PRO badge + 生效中 */}
            <div className="flex items-center gap-2">
              <span className="flex items-center gap-1 rounded-full bg-white/70 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.12em] text-[#c96442] shadow-sm ring-1 ring-inset ring-[#c96442]/25 backdrop-blur">
            ...
            <div className="mt-3 flex items-baseline gap-1.5">
              <span className="text-[40px] font-semibold leading-none tracking-tight text-[#141413] tabular-nums">
                $99
              </span>
            ...
            return (
              <div key={u.label}>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="ml-auto text-xs tabular-nums text-[#141413]">
                    <span className="font-medium">{u.used}</span>
                    <span className="text-[#87867f]"> / {u.quota} {u.unit}</span>
                  </span>
                </div>
                {/* Row 2: progress bar */}
                ...
                {isWarn && (
                  <p className="mt-1.5 flex items-center gap-1 text-[10px] font-medium text-[#d97706]">
                    <AlertTriangle className="h-3 w-3" />即将用尽
                  </p>
                )}
              </div>
            );
      ...
      <div className="rounded-2xl border border-[#e8e6dc] bg-white p-6">
        ...
        <div className="grid grid-cols-[100px_1fr_90px_108px_68px] gap-3 border-b border-[#e8e6dc] pb-2 text-[11px] font-medium uppercase tracking-wide text-[#87867f]">
          <span>日期</span><span>项目</span><span className="text-right">金额</span><span>状态</span><span className="text-right">发票</span>
        </div>
        {BILLING_HISTORY.map((b, i) => (
          ...
          >
            ...
            <span className="flex items-center gap-1 text-xs font-medium text-[#5a6a4a]">
              <Check className="h-3 w-3" />{b.status}
            </span>
            <button
              type="button"
  ...
  return (
    <div className="w-full space-y-4">
      <div className="rounded-2xl border border-[#e8e6dc] bg-white p-5">
        ...
        <p className="mt-3 text-xs text-[#87867f]">通过插件浏览 TikTok / Instagram 时自动同步博主数据到博主库，支持一键收藏、快速建联。</p>
      </div>
      <div className="rounded-2xl border border-[#e8e6dc] bg-white p-5">
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-3">
        ...
        <p className="mt-3 text-xs text-[#87867f]">管理邮箱发件和收件功能。在建联中心 → 邮件管理中配置详细选项。</p>
      </div>
