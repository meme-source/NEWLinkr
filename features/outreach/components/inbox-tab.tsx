...
import { EmailSettingsDrawer } from "@/features/outreach/components/mailbar/email-settings-drawer";
...
//
// §3.6.2 (refactor 2026-05): the mailbar above the inbox is replaced with a
// thin header strip on the inbox frame itself. Settings now live in a right-side
// drawer (<EmailSettingsDrawer />) instead of the old expand-down panel.
  ...
  return (
    <>
      <div className="overflow-hidden rounded-lg border border-[#c5c0b1] bg-[#fffefb]">
        <InboxMailbar account={settings.account} onOpenSettings={openSettings} />

        <div className="flex min-h-[480px]">
          <aside className="flex w-72 shrink-0 flex-col overflow-y-auto border-r border-[#c5c0b1]">
          ...
          <div className="flex min-w-0 flex-1 flex-col bg-[#fffefb]">
            {activeThread ? (
              <>
                ...
                <div className="flex-1 space-y-4 overflow-y-auto p-5">
                  <div className="flex justify-end">
                    <div className="max-w-sm rounded-lg rounded-tr-sm bg-[#201515] px-4 py-3 text-sm text-[#fffefb]">
                      <p className="font-medium">合作邀请 — MyBrand 护肤新品</p>
                      <p className="mt-2 leading-relaxed text-[#fffefb]/80">
                        Hi {activeThread.handle.replace("@", "")}! I&apos;m reaching out from
                        MyBrand. We&apos;ve been following your content and would love to
                        collaborate on our new skincare launch...
                      </p>
                      <p className="mt-2 text-xs text-[#fffefb]/50">
                        04-10 · marketing@mybrand.com
                      </p>
                    </div>
                  </div>
                  {activeThread.id === 1 && (
                    <div className="flex justify-start">
                      <div className="max-w-sm rounded-lg rounded-tl-sm border border-[#c5c0b1] bg-[#eceae3] px-4 py-3 text-sm text-[#201515]">
                        <p className="leading-relaxed">
                          Thanks for reaching out! I&apos;d love to learn more about the
                          collaboration. Could you share the campaign brief and compensation
                          details?
                        </p>
      ...
      <EmailSettingsDrawer open={settingsOpen} onClose={closeSettings} settings={settings} />
    </>
  );
}
