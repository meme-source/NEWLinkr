...
function getCreatorContactEmail(creator: CreatorProfile) {
  return creator.email ?? null;
}

function getCreatorAveragePlays(creator: CreatorProfile, scrapeCount?: number) {
...
function getCreatorMedianPlays(creator: CreatorProfile, scrapeCount?: number) {
  const averagePlays = parseMetricToNumber(getCreatorAveragePlays(creator, scrapeCount));
  return formatPlays(averagePlays * 0.82);
}

function getCreatorCpm(creator: CreatorProfile) {
  return creator.cpm ?? "$10.00";
}

function getCreatorMedianComments(creator: CreatorProfile) {
...
function getCreatorMetricSnapshot(creator: CreatorProfile, scrapeCount?: number) {
...
function getCreatorReview(creator: CreatorProfile) {
  return creator.review ?? "评论区互动自然，广告痕迹低，品牌合作接受度较高。";
}

function getCreatorDiagnostics(creator: CreatorProfile) {
...
function getCreatorLocation(creator: CreatorProfile) {
...
function getCreatorType(creator: CreatorProfile) {
...
function getCreatorAudienceBreakdown(creator: CreatorProfile) {
  return (
...
function getRegionTierLabel(tier: RegionTierKey) {
...
function parseMetricToNumber(value: string): number {
...
function CreatorTopicSummaryRow({
  topics,
  scrapeCount,
}: {
  topics: Array<{ label: string; weight: number }>;
